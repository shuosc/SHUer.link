<template>
  <div class="login">
    <mu-drawer right :docked="false" :open="this.$store.state.sidebarState" @close="toggleSidebar" :width="300">
      <mu-appbar title="登录" />
      <div class="center">
        <h3>登录</h3>
        <mu-avatar :size="80" />
      </div>
      <mu-text-field label="一卡通账号" @textOverflow="inputError" :errorText="inputErrorText" labelFloat icon=":iconfont icon-account" :maxLength="8" v-model="student.card_id" type="string" />
      <div id="password">
        <mu-text-field label="密码" :type="passwordVisible?'text':'password'" labelFloat v-model="student.password" icon=":iconfont icon-password" />
